...
src/
  ...
  tui/
    ...
    app.rs             AppState, TuiApp, ChatMessage, agent stream dispatch
    event.rs           Unified Event enum (terminal + agent events)
    ...
    widgets/
      ...
      status_bar.rs    Model name, spinner, turn count, scroll indicator, key hints
...
- **Built-in commands**: `/exit` (`/quit`), `/clear` (`/reset`, `/new`), `/help` (`/?`), `/status`, `/compact <instructions>`
...
- **update_suggestions()** (`app.rs`): Called after every keystroke. Preserves selected item across re-filters.
- **handle_key()** (`mod.rs`): Routes Tab (accept suggestion), Up/Down (navigate), Esc (dismiss), Enter (accept+execute or submit).
